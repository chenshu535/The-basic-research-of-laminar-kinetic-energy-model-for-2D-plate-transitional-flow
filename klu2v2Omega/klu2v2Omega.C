...
defineTypeNameAndDebug(klu2v2Omega, 0);
addToRunTimeSelectionTable(RASModel, klu2v2Omega, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

tmp<volScalarField> klu2v2Omega::fv(const volScalarField& Ret) const
{
    return(1.0 - exp(-sqrt(Ret)/Av_));
}


tmp<volScalarField> klu2v2Omega::fINT() const
{
    return
    (
        min
        (
            (u2_ + v2_)/(Cint_*(kl_ + (u2_ + v2_) + kMin_)),
...
tmp<volScalarField> klu2v2Omega::fSS(const volScalarField& Omega) const
{
    return(exp(-sqr(Css_*nu()*Omega/((u2_ + v2_) + kMin_))));
}


tmp<volScalarField> klu2v2Omega::Cmu(const volScalarField& S) const
{
    return(1.0/(A0_ + As_*(S/(omega_ + omegaMin_))));
}


tmp<volScalarField> klu2v2Omega::BetaTS(const volScalarField& ReOmega) const
{
    return(scalar(1) - exp(-sqr(max(ReOmega - CtsCrit_, scalar(0)))/Ats_));
}


tmp<volScalarField> klu2v2Omega::fTaul
(
...
tmp<volScalarField> klu2v2Omega::alphaT
(
    const volScalarField& lambdaEff,
...
tmp<volScalarField> klu2v2Omega::fOmega
(
    const volScalarField& lambdaEff,
...
tmp<volScalarField> klu2v2Omega::phiBP(const volScalarField& Omega) const
{
    return
    (
        min
...
tmp<volScalarField> klu2v2Omega::phiNAT
(
...
tmp<volScalarField> klu2v2Omega::D(const volScalarField& k) const
{
...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("u2", alphaRhoPhi.group()),
            runTime_.timeName(),
            mesh_,
            IOobject::MUST_READ,
            IOobject::AUTO_WRITE
    ...
    (
        ...
        (u2_ + v2_)*omega_ + D(kl_) + D(u2_) + D(v2_)
    ),
...
void klu2v2Omega::validate()
{}


void klu2v2Omega::correct()
{
    ...
    const volScalarField lambdaT(sqrt((u2_ + v2_))/(omega_ + omegaMin_));
    ...
    const volScalarField ktS(fSS(Omega)*fw*(u2_ + v2_));

    const volScalarField nuts
    (
        fv(sqr(fw)*(u2_ + v2_)/nu()/(omega_ + omegaMin_))
       *fINT()
       *Cmu(sqrt(S2))*sqrt(ktS)*lambdaEff
    );
    volScalarField Pkt(this->GName(), nuts*S2);

    const volScalarField ktL((u2_ + v2_) - ktS);
    ...
    (
        alphaT(lambdaEff, fv(sqr(fw)*(u2_ + v2_)/nu()/(omega_ + omegaMin_)), ktS)
    );
    ...
    (
     ...
     ==
        Cw1_*Pkt*omega_/((u2_ + v2_) + kMin_)
      - fvm::SuSp
        (
            (1.0 - CwR_/(fw + fwMin))*kl_*(Rbp + Rnat)/((u2_ + v2_) + kMin_)
          , omega_
        )
      - fvm::Sp(Cw2_*sqr(fw)*omega_, omega_)
      + (
            Cw3_*fOmega(lambdaEff, lambdaT)*alphaTEff*sqr(fw)*sqrt((u2_ + v2_))
        )()()/pow3(y_())
    );
    ...
    const volScalarField Du2(D(u2_));

    // streamwise turbulent kinetic energy equation
	tmp<fvScalarMatrix> u2Eqn
	(
	    fvm::ddt(u2_)
      + fvm::div(phi_, u2_)
      - fvm::laplacian(DkEff(alphaTEff), u2_)
     ==
        Pkt
      - fvm::Sp(omega_ + Du2/(u2_+ kMin_), u2_)
	);


    u2Eqn.ref().relax();
    u2Eqn.ref().boundaryManipulate(u2_.boundaryFieldRef());

    solve(u2Eqn);
    bound(u2_, kMin_);
	
	// wall normal turbulent kinetic energy
	const volScalarField Dv2(D(v2_));
    tmp<fvScalarMatrix> v2Eqn
	(
	    fvm::ddt(v2_)
      + fvm::div(phi_, v2_)
      - fvm::laplacian(DkEff(alphaTEff), v2_)
     ==
      (Rbp + Rnat)*kl_
      - fvm::Sp(omega_ + Dv2/(v2_+ kMin_), v2_)
	);
    ...
    epsilon_ = (u2_ + v2_)*omega_ + Du2 + Dv2;
    bound(epsilon_, epsilonMin_);
