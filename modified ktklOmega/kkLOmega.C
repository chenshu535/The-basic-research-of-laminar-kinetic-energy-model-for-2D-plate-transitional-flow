...
tmp<volScalarField> kkLOmega::phiBP(const volScalarField& Omega, const volScalarField& ReOmega) const
{
...
*/
//correction for sensibility of pressure gradient 
tmp<volScalarField> kkLOmega::D(const volScalarField& k) const
{
    return nu()*magSqr(fvc::grad(sqrt(k)));
}

tmp<volScalarField> kkLOmega::Ue(const volScalarField& p, const volVectorField& U) const
{
...
tmp<volScalarField> kkLOmega::L(const volScalarField& ReOmega) const
{
...
tmp<volScalarField> kkLOmega::CtsCrit(const volScalarField& L) const
{
    return tmp<volScalarField>(new volScalarField(
        "CtsCrit",
...
tmp<volScalarField> kkLOmega::CnatCrit(const volScalarField& L) const
{
    return tmp<volScalarField>(new volScalarField(
            "CnatCrit",
           // this->CnatCrit_ / ( scalar(1) - this->CnatApg_ * min(L, scalar(0)) )
            this->CnatCrit_ / ( scalar(1) - 0.2 *min( max(L, scalar(0)), scalar(2)) )
        ));
}

tmp<volScalarField> kkLOmega::CbpCrit(const volScalarField& L) const
{
    return tmp<volScalarField>(new volScalarField(
            "CbpCrit",
            scalar(1.2) +0* min(max(L,scalar(0)), scalar(1))
        ));
}
...
void kkLOmega::correct()
{
    ...
    (
        CR_*(1.0 - exp(-phiBP(Omega, ReOmega)()/Abp_))*omega_
       /(fw + fwMin)
    );
