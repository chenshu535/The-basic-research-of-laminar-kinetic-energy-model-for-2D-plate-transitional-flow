...
dimensionedScalar U0_(dimensionSet(0,1,-1,0,0 ,0, 0),1e-8);
dimensionedScalar I0(dimensionSet(0,-1,0,0,0 ,0, 0),1);
//volScalarField I0_(I0);
//volScalarField Tu0_=mesh().C().component(0).dimensions().reset(dimless);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //
   ...
   strainRate_
    (
        ...
        (
           ...
           sqrt(2.0*magSqr(dev(symm(fvc::grad(U_)))))
      ),
     rotatingRate_
    (
        IOobject
    ...
    (
       ...
       mag(U_ & (fvc::grad(mag(U_))))/(mag(U_)+mag(U0_))
    ),
     Tu_
    (
        IOobject
        (
         ...
         0.02068+0.07062*exp((-0.31189-mag(mesh().C().component(0))*I0*sign(mesh().C().component(0)))/0.1027)+0.05422*exp((-0.31189-mag(mesh().C().component(0))*I0*sign(mesh().C().component(0)))/0.73216)
...
void kkLOmega::correct()
{
    ...
    strainRate_=sqrt(2.0*magSqr(dev(symm(fvc::grad(U_)))));
    rotatingRate_=sqrt(2.0)*mag(skew(fvc::grad(U_)));
    dUds_=mag(U_ & (fvc::grad(mag(U_))))/(mag(U_)+mag(U0_));
  
    volScalarField I1_(mesh().C().component(0)-mesh().C().component(0));
    volScalarField I2_(mesh().C().component(0));
    Tu_=0.02068+0.07062*exp((-0.31189-mag(mesh().C().component(0))*I0*sign(mesh().C().component(0)))/0.1027)+0.05422*exp((-0.31189-mag(mesh().C().component(0))*I0*sign(mesh().C().component(0)))/0.73216);

    // Re-calculate turbulent viscosity
    convecl_=-fvc::div(phi_,kl_);
    PKl_=Pkl;
    Dil_=-fvc::Sp(Dl/(kl_+ kMin_), kl_);
    Dfl_=fvc::laplacian(nu(), kl_);
    RNAT_=Rbp;
    RBP_=Rnat;
    convect_=-fvc::div(phi_,kt_);
    PKt_=Pkt;
    epsilont_=-fvc::Sp(omega_, kt_);
    Dit_=-fvc::Sp(Dt/(kt_+ kMin_), kt_);
    Dft_=fvc::laplacian(nu(), kt_);
